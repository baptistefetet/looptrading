...
## Tasks / Subtasks
- [x] Créer table AlertRule (AC: 1)
  - [x] Migration Prisma
  - [x] Champs : id, strategy (enum), params (JSON), enabled, createdAt
  - [x] Stratégies : PULLBACK, BREAKOUT, MACD_CROSS
- [x] Implémenter stratégie Pullback (AC: 2)
  - [x] Détection tendance haussière (prix > SMA200)
  - [x] Prix proche SMA50 (dans les 2%)
  - [x] RSI entre 40-50 (zone de pullback)
  - [x] Volume en baisse (repos)
- [x] Implémenter stratégie Breakout (AC: 2)
  - [x] Prix dépasse résistance récente
  - [x] Volume > 150% moyenne 20j
  - [x] Confirmation : clôture au-dessus résistance
- [x] Implémenter stratégie MACD Cross (AC: 2)
  - [x] Croisement ligne MACD au-dessus Signal
  - [x] Histogramme passe positif
  - [x] Confirmer avec tendance (prix > SMA50)
- [x] Créer table Alert (AC: 3)
  - [x] Migration Prisma
  - [x] Champs : id, symbol, strategy, score, triggeredAt, acknowledged, acknowledgedAt
  - [x] Index sur triggeredAt et acknowledged
- [x] Créer job d'évaluation (AC: 4)
  - [x] Fonction `evaluateAlerts()` async
  - [x] Enregistrer dans SchedulerService avec cron `*/15 * * * *`
  - [x] Pour chaque action active, évaluer chaque règle enabled
  - [x] Créer alerte si conditions remplies
- [x] Implémenter déduplication (AC: 5)
  - [x] Vérifier existence alerte récente (< 24h)
  - [x] Même symbol + même stratégie
  - [x] Skip si doublon
...
| 2026-02-08 | 1.0 | Implémentation du moteur d'alertes (stratégies, déduplication 24h, job cron, API alerts, tests) | Dev |
