...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed left-0 top-0 z-30 h-full w-64 transform border-r border-dark-600 bg-dark-800 transition-transform duration-300 lg:translate-x-0 ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        {/* Logo */}
        <div className="flex h-16 items-center border-b border-dark-600 px-6">
          <span className="text-xl font-bold text-neon-green">Loop</span>
          <span className="text-xl font-bold text-gray-100">Trading</span>
        </div>
        ...
        {/* Version */}
        <div className="absolute bottom-4 left-0 right-0 px-6 text-center text-xs text-gray-600">
          v0.1.0
        </div>
      </aside>
    </>
