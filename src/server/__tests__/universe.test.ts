...
describe('Universe endpoints', () => {
  ...
  describe('POST /api/stocks/import', () => {
    it('should import stocks from CSV with header', async () => {
      const csv = `symbol,market,sector
AAPL,US,Technology
MSFT,US,Technology
MC.PA,EU,Luxury`;
      ...
      expect(stocks[0].market).toBe('US');
      expect(stocks[0].sector).toBe('Technology');
    });

    it('should import stocks from CSV without header', async () => {
      const csv = `AAPL,US,Technology
MSFT,US,Technology`;
    ...
    it('should import simple symbol-only CSV', async () => {
      const csv = `AAPL
MSFT
GOOGL`;
      ...
      // Verify auto-detected market
      const aapl = await prisma.stock.findUnique({ where: { symbol: 'AAPL' } });
      expect(aapl!.market).toBe('US');
    });

    it('should skip already active stocks', async () => {
      await prisma.stock.create({
        data: { symbol: 'AAPL', name: 'Apple Inc.', market: 'US', active: true },
      });
...
MSFT,US,Technology`;
    ...
    it('should reactivate inactive stocks', async () => {
      await prisma.stock.create({
        data: { symbol: 'AAPL', name: 'Apple Inc.', market: 'US', active: false },
      });
    ...
    it('should reject import exceeding 500-stock limit', async () => {
      // Create 499 active stocks
      const stocks = Array.from({ length: 499 }, (_, i) => ({
        symbol: `STK${String(i).padStart(3, '0')}`,
        name: `Stock ${i}`,
        market: 'US',
        active: true,
      ...
      // Try to import 2 more (would exceed 500)
      const csv = `AAPL,US,Technology
MSFT,US,Technology`;
